var blurbGallery={init:function(a,b){var c=this;c.config=$.extend({},c.defaults,a);c.elem=b;c.$elem=$(b);$.when(c.load()).then(function(){c.render.call(c);c.events()},function(){alert("Error: Gallery load failed!")});return c},defaults:{ajaxCache:!1,ajaxDataType:"json",ajaxDataUrl:"json/jquery.single.json",callback:function(){},classActivePage:"bg-active-page",classActiveThumb:"bg-active-thumb",classCategory:"bg-category",classDescription:"bg-description",classEnlarge:"bg-enlarge",classGallery:"bg-gallery",
...
$(this));b.preventDefault()});a.$elem.on("click","."+a.config.classPreviousLink,function(b){a.changePage.call(a,$(this),"previous");b.preventDefault()});a.$elem.on("click","."+a.config.classPageLink,function(b){a.changePage.call(a,$(this));b.preventDefault()});a.$elem.on("click","."+a.config.classNextLink,function(b){a.changePage.call(a,$(this),"next");b.preventDefault()})},changeCategory:function(a){var b=this;a=b.utils.formatText(a);$("."+b.config.classImgCtn).hide();$("."+b.config.classInfo).hide();
b.config.loading&&$("."+b.config.classLoading).show();b.config.selectedCat=a;b.selected=b.findSelected();b.renderTitle($("."+b.config.classTitle));b.renderCategory($("."+b.config.classCategory));b.renderDescription($("."+b.config.classDescription));a=b.renderImage($("."+b.config.classImg));b.renderThumbs($("."+b.config.classThumbs));$.when(b.utils.preload(a),b.utils.preload(a)).then(function(){b.config.loading&&$("."+b.config.classLoading).hide();$("."+b.config.classImgCtn).fadeIn(b.config.speed);
